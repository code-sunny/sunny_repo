...
async function songLike(event) {
  ...
  try {
    const {likes} = await (await fetch("/api/like-btn", {
      method: "POST",
      headers: {
...
[...moveBtns].map(btn => btn.addEventListener("click", (event) => showRank(event)));

const weatherLikeBtns = document.querySelectorAll(".weather-like-btn");
[...weatherLikeBtns].map(btn => btn.addEventListener("click", (event) => songLike(event)));